...
class PolOdm
{
    ...
    // PHP4
    public function PolOdm() { $this->__construct(); }
    
    // Dodaje link do konfiguracji wtyczki
    public function dodaj_opcje()
    {
        // Wyświetla link
    ...
    public function wyswietl_opcje()
    {
...
<?php        
    ...
    public function rozloz_liczbe( $liczba = 0 )
    {
    ...
    public function zamien_na_slowa( $rozlozona )
    {
    ...
    public function odmieniaj( $rozlozona )
    {
    ...
    // Wyświetla wszystko
    public function pokaz( $pierwszy = false, $drugi = false, $trzeci = false, $lnumber = 0 )
    {
    ...
    function data( $d = '' )
    {
        if( get_option( 'polodm_uzyj_daty' ) == 'no' )
        {
            // proszę mi wytłumaczyć, czemu to się zapętla w nieskończność -> piotr@lysiu.pl
        ...
        // narazie, więc godzina jest po prostu wyświetlana normalnie 
        if( preg_match( "/\d*:\d*.?.?.?/", $d ) ) 
        {
            echo $d;
            return;
